...
const TeamBlogDashboard = () => {
    ...
    const fetchBlogs = async () => {
      try {
        ...
        if (response.data.success) {
          setBlogs(response.data.data.filter(blog => blog.author && blog.author._id));
        } else {
  ...
  const handleDelete = async (blogId) => {
    ...
    if (!blog.author || blog.author._id.toString() !== (userData.id || userData._id).toString()) {
  ...
  const handlePublish = async (blogId) => {
    const blog = blogs.find((b) => b._id === blogId);
    if (!blog.author || blog.author._id.toString() !== (userData.id || userData._id).toString()) {
      toast.error("You can only publish your own blogs.");
      return;
    }
    try {
  ...
  const openModal = (blog = null) => {
    if (blog && (!blog.author || blog.author._id.toString() !== (userData.id || userData._id).toString())) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {blogs.length === 0 ? (
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {blogs.map((blog) => {
              console.log("Blog Author ID:", blog.author?._id?.toString());
              console.log("UserData ID:", userData.id || userData._id);
              console.log("Match:", blog.author?._id?.toString() === (userData.id || userData._id)?.toString());
              return (
                <div
                  key={blog._id}
                ...
                >
                  ...
                  <div className="text-gray-500 text-xs mb-4">
                    ...
                    <p>Author: {blog.author?.name || "Unknown"} ({blog.author?.role || "Unknown"})</p>
                    {blog.tags.length > 0 && (
                      <p className="mt-1">
                        Tags:{" "}
                        {blog.tags.map((tag) => (
                  ...
                  <div className="flex justify-end space-x-3">
                    {blog.author && blog.author._id && blog.author._id.toString() === (userData.id || userData._id)?.toString() && (
                      <>
                        <button
